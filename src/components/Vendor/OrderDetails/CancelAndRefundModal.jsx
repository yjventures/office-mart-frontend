...
const CancelAndRefundModal = ({ setConfirmModal, info, cancelId }) => {
    ...
    const handleCancelOrder = async () => {
        ...
        // console.log(myReason?.title)

        try {
            const response = await axios.post(`${import.meta.env.VITE_API_PATH}/refunds/create`,
                {
                    "reason": myReason?.title,
                    "order": info?.order_id?._id,
                    "product_item": info?.order_id?.items[0]?._id,
                    "shop": info?.shop_id?._id,
...
export default CancelAndRefundModal