...
const Products = () => {
    ...
    // count total and unit price
    const [totalPrice, setTotalPrice] = useState(calculateTotalPrice(product, quantity, selectedVariationDetails));
    // console.log(totalPrice)
    useEffect(() => {
        setTotalPrice(calculateTotalPrice(product, quantity, selectedVariationDetails))
    }, [product, quantity, selectedVariationDetails])

    const [unitPrice, setUnitPrice] = useState(calculateUnitPrice(product, quantity, selectedVariationDetails));
    useEffect(() => {
        setUnitPrice(calculateUnitPrice(product, quantity, selectedVariationDetails))
    }, [product, quantity, selectedVariationDetails])
                        ...
                        {relatedProductsData.map((item, index) => {
                            ...
                            return (
                                ...
                                <div key={index} className='ms-2 relative border p-3 flex flex-col items-center gap-3 border-gray-200 w-[232px] rounded-md mx-auto'>
                                    ...
                                    <p className='text-[16px] font-[600] text-tertiary-600'> {item?.price} USD </p>
    ...
    const handleBuyNow = async () => {
        ...
        const total_price = Number(totalPrice);
        const product_varient = selectedVariationDetails?._id;
        const userId = JSON.parse(localStorage.getItem('user'))?._id;
        try {
            ...
            if (response.status === 200) {
                ...
                const cartItems = {
                    user_id: userId,
                    items: [newItem],
                    sub_total: total_price, // Adjust if you're considering quantity
                    tax: 0,
                    total_price: total_price,
                    comment: "",
                    shipping: 0,
    ...
    const RenderVendorInfo = () => {
        ...
        return (
            <div className='md:flex gap-2 items-center'>
                <img
                    alt="Banner"
                    className="aspect-[2/1] overflow-hidden rounded-lg object-contain object-center w-full h-[300px] "
                    // height="200"
                    src={shop?.banner}
                    width="400"
                />
                <div className='w-full flex justify-between items-center h-[300px] flex-col'>
                    ...
                    <div className='flex w-full gap-3'>
                        ...
                        {/* chat button */}
                        <button
                            onClick={() => navigate(`/user-single-chat?shopId=${shop._id}&shopName=${shop?.name?.en}&banner=${shop?.banner}`)}
                        ...
                        <button
                            onClick={() => navigate(`/vendor-page?name=${encodeURIComponent(shop?.name?.en)}`)}
                            className='bg-black text-white px-4 py-2 mt-10 md:mt-0 w-full rounded-md content-center flex justify-center items-center gap-2'
                        >
                        ...
                        <div className='grid grid-cols-1 lg:grid-cols-2 gap-7'>
                            ...
                            <div className='grid md:grid-cols-6 grid-cols-1'>
                                    ...
                                    {/* Map over product images */}
                                    {
                                        getImages().map((image, index) => (
                                            <div key={index}
                                                ...
                                                <img src={image} alt={`Image ${index + 1}`} className='w-full h-full rounded-md p-[1px] object-contain' />
                                            </div>
                                        ))
                                    }
                                </div>
                                ...
                                <div className='col-span-4 flex justify-center items-center border-[1px] relative sm:min-h-[600px] sm:max-h-[650px] rounded-md md:order-1'>
                                    ...
                                    {/* Discount label */}
                                    {salesPrice && (
                                        <p className='absolute top-3 left-3 bg-tertiary-600 px-3 py-1 text-primary-color rounded-md'>
                                            {Math.round(percentageDiscount)}% off
                                        </p>
                                    )}

                                    {/* Selected image */}
                                    <img src={selectedImage} alt="Selected-image" className='sm:h-full h-[400px] object-contain rounded-md' />
                            ...
                            <div>
                                ...
                                <div className='flex items-center gap-3 mt-6'>
                                    ...
                                    <div className='w-[40px] h-[40px] flex justify-center items-center rounded-md cursor-pointer text-secondary-color'>
                                        <input
                                            type="number"
                                            className='w-10 h-10 border-[1px] rounded-md flex items-center justify-center text-[16px] font-[600] border-gray-100 text-center outline-none'
                                            value={quantity}
                                            onChange={(e) => {
                                                const value = parseInt(e.target.value, 10); // Convert the input value to a number
                                                if (isNaN(value) || value < 1) {
                                                    setQuantity(1); // Set to 1 if input is invalid or less than 1
                                                } else {
                                                    setQuantity(value); // Update the state with the valid number
                                                }
                                            }}
                                        />
                                    </div>
                                ...
                                <div className='w-full mt-6' >
                                    <div onClick={() => {
                                        if (isLoadingBuyNow == false) {
                                            handleBuyNow();
                                        } else {
                                            showToast('Hey wait! we are processing your product', 'info')
                                        }
                                    }} className='border border-gray-100 py-2 flex justify-center gap-2 items-center rounded-md cursor-pointer text-primary-color hover:bg-buttons bg-secondary-color hover:text-secondary-color transition'>
                                    ...
                                    <div
                                        onClick={() => {
                                            if (user) {
                                                ...
                                                showToast('Sign in to add to wishlist', 'info')
                                                navigate('/signin')
                                            }
                                        }}
